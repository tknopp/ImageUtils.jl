...
@test all(isapprox.(ImageUtils.normalizeGray(colormap[1]), RGB{Float64}(0.8979049716100672,0.6795060499249219,0.28645928277939586), rtol=1e-10))
@test @test_logs (:warn,"Normalization of gray value failed, g ≈ 0.056 is used. Use g >= 0.056 for a consistent normalization.") match_mode=:any all(isapprox.(ImageUtils.normalizeGray(colormap[1],0.05), RGB{Float64}(0.18396207457891003,0.0,0.0), rtol=1e-10))
@test @test_logs (:warn,"Normalization of gray value failed, g ≈ 0.925 is used. Use g <= 0.925 for a consistent normalization.") match_mode=:any all(isapprox.(ImageUtils.normalizeGray(colormap[1],0.95), RGB{Float64}(1.0,0.9603124734843839,0.5548057892335283), rtol=1e-10))
